...
void free_fs_objects() {
    for (int i = 0; i < MAX_FS_OBJECTS; i++) {
        if (fs_objects[i].type != NULL) {
            free((void*)fs_objects[i].type);
            free(fs_objects[i].name);
            free(fs_objects[i].data);
            json_object_put(fs_objects[i].entries);
        }
    }
}
...
void add_free_inode(int inode) {
    ...
    if (num_free_inodes % MAX_FS_OBJECTS == 0) {
        free_inodes = realloc(free_inodes, (num_free_inodes + MAX_FS_OBJECTS) * sizeof(int));
        if (!free_inodes) {
...
static int load_json_fs(const char *filename) {
...
static void fuse_example_destroy(void *private_data) {
    (void) private_data;
    store_file_system("fs.json");
    free_fs_objects();
}

int lookup_inode(const char *path) {
    char *path_copy = strdup(path);
    char *saveptr;
    char *component = strtok_r(path_copy, "/", &saveptr);

    // Start at the root directory.
    fs_object *current_directory = &fs_objects[0];  // Assuming the root directory is the first object in the array.

    while (component != NULL) {
        bool found = false;

        // Iterate over the entries in the current directory.
        struct json_object *entry_list = current_directory->entries;
        ...
        for (int i = 0; i < num_entries; i++) {
            ...
            struct json_object *name_obj;
            json_object_object_get_ex(entry_obj, "name", &name_obj);
            const char *name = json_object_get_string(name_obj);

            if (strcmp(name, component) == 0) {
                // This is the entry we're looking for. Update current_directory and break out of the loop.
                struct json_object *inode_obj;
                json_object_object_get_ex(entry_obj, "inode", &inode_obj);
                int inode = json_object_get_int(inode_obj);

                current_directory = &fs_objects[inode];
                found = true;
                break;
            }
        }
        ...
        if (!found) {
            // We didn't find the entry for the current component of the path. The file or directory does not exist.
            free(path_copy);
            return -1;
        }
    ...
    free(path_copy);
    return current_directory->inode;
}
...
static int fuse_example_read(const char *path, char *buf, size_t size, off_t offset,
    ...
    if (obj->type && strcmp(obj->type, "reg") == 0 && obj->data) {
...
static int fuse_example_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    pthread_mutex_lock(&fs_mutex);  // Add mutex lock for concurrency control


    int inode = lookup_inode(path);
    if (inode < 0) {
        pthread_mutex_unlock(&fs_mutex);  // Release lock before returning
        return -ENOENT;
    }// No such file or directory

    const fs_object *obj = &fs_objects[inode];
    char *temp_path = strdup(path);
    if(strcmp(obj->type, "dir") != 0 && strcmp(obj->name, basename(temp_path)) == 0) {
    ...
    free(temp_path);
    pthread_mutex_unlock(&fs_mutex);  // Release lock at the end of function
    return 0;
}
...
static int fuse_example_write(const char *path, const char *buf, size_t size, off_t offset,
    ...
    if (obj->data) {
        ...
        if (new_size > strlen(obj->data)) {
            // Reallocate memory and check for allocation failure
            char *new_data = realloc(obj->data, new_size + 1);  // +1 for the null terminator
...
static int fuse_example_create(const char *path, mode_t mode, struct fuse_file_info *fi) {
    ...
    free(temp_path);  // Free the memory allocated to temp_path
...
static int fuse_example_truncate(const char *path, off_t newsize) {
    ...
    if (obj->data) {
        if (newsize < strlen(obj->data)) {
            // If newsize is less than the current data size, reduce the size of the data.
            char *new_data = realloc(obj->data, newsize + 1);
            if (!new_data) return -ENOMEM;
            obj->data = new_data;
            obj->data[newsize] = '\0'; // Ensure the new data is null-terminated
        } else if (newsize > strlen(obj->data)) {
            // If newsize is more than the current data size, increase the size of the data.
            char *new_data = realloc(obj->data, newsize + 1);  // +1 for the null terminator
            if (!new_data) return -ENOMEM;
            obj->data = new_data;
...
static int fuse_example_mkdir(const char *path, mode_t mode) {
    ...
    new_obj->type = "dir";
    char *temp_path = strdup(path);
    new_obj->name = strdup(basename(temp_path));  // Use the base name of the path
    free(temp_path);
...
static int fuse_example_unlink(const char *path) {
    ...
    // Free the fs_object's resources.
    free(fs_objects[inode].name);
    if (fs_objects[inode].data) {
        free(fs_objects[inode].data);
    }
    if (fs_objects[inode].entries) {
        json_object_put(fs_objects[inode].entries);
    }

    // Remove the fs_object from the parent directory's entries.
    char *parent_path = strdup(path);
    dirname(parent_path);
    int parent_inode = lookup_inode(parent_path);
    free(parent_path);
    fs_object *parent_obj = &fs_objects[parent_inode];
    struct json_object *entry_list = parent_obj->entries;
    int num_entries = json_object_array_length(entry_list);
    ...
    for (int i = 0; i < num_entries; i++) {
        ...
        json_object_object_get_ex(entry_obj, "name", &name_obj);
        const char *name = json_object_get_string(name_obj);
        if (strcmp(name, fs_objects[inode].name) != 0) {
            // This is not the entry to remove, add it to the new array.
            json_object_array_add(new_entry_list, json_object_get(entry_obj));
        }
    }

    json_object_put(entry_list);  // Free the old array.
    parent_obj->entries = new_entry_list;  // Replace the old array with the new one.

    // Remove the fs_object from the fs_objects array.
    memmove(&fs_objects[inode], &fs_objects[inode+1], sizeof(fs_object) * (num_fs_objects - inode - 1));
    num_fs_objects--;
    fs_objects = realloc(fs_objects, num_fs_objects * sizeof(fs_object));
...
static int fuse_example_rmdir(const char *path)
{
    ...
    if(fs_objects[inode].type == NULL || strcmp(fs_objects[inode].type, "dir") != 0) {
        return -ENOTDIR;
    }
...
int main(int argc, char *argv[]) {
    if(pthread_mutex_init(&fs_mutex,NULL) != 0){
        printf("Mutex init failed\n");
        return -1;
    }

    if(load_json_fs("fs.json") != 0){
        printf("load_json_fs failed\n");
        return -1;
    }
